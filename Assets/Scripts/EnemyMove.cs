...
public class EnemyMove : MonoBehaviour {
	...
	void Update () {
        ...
        {
            if (enemy.transform.position != waypoints[next_waypoint].transform.position && 
                this.gameObject.name != "Enemy" && this.gameObject.name != "Enemy1" && 
                this.gameObject.name != "Enemy2" && this.gameObject.name != "Enemy3")
            {
                Vector2 new_pos = Vector2.Lerp(position, waypoints[next_waypoint].transform.position, t);
                enemy.transform.position = new_pos;
            }
            else if (enemy.transform.position == waypoints[next_waypoint].transform.position)
            {
                next_waypoint++;
                position = enemy.transform.position;
                t = 0;
